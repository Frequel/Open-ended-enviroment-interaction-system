...
public class interactableChecker2D : MonoBehaviour
{
    ...
    {
        //Collider[] hitColliders = Physics2D.OverlapBox(overlapBoxCenDito, overlapBoxDimDito / 2, Quaternion.identity, m_LayerMask).OrderBy(c => c.transform.position.z).Where(c => c.transform.position.z > transform.position.z).ToArray();
        Collider2D hitCollider = Physics2D.OverlapBox(overlapBoxCenDito, overlapBoxDimDito / 2, 0, m_LayerMask);//.OrderBy(c => c.transform.position.z).Where(c => c.transform.position.z > transform.position.z).ToArray();
